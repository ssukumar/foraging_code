function [T] = robotdataread(filename,filedir, logfnid)
...
%                 else
                    T.framedata(trialno(i))=robotconv(fid, logfnid);
                    fprintf('Loop Number: %d; Trial %d/%d \n',i,trialno(i),ntrials);
                    T.trial_arr(i) = i;%trialno(i);
%                 end
                fclose(fid);
            else
...
for i=1:length(fpzeroindex)
            ...
            if fid > 0
                T.fpzerodata(i)=robotconv(fid,logfnid);
