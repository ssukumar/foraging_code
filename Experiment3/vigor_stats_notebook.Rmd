...
data = read.csv(file='expeirment3_data')
...
```
```{r}
# aov_agg = aggregate(data=data, pv_diff ~ Subj_id+Env+Probe+block_no+Order, mean )
# nrow(aov_agg)
# pv_agg.aov =  aov(pv_diff~Env*Probe + Order + Error(Subj_id), data = aov_agg)
# 
# # pv_agg.aov <-with(data=aov_agg , pv_agg.mdl)
# summary(pv_agg.aov)
```
...
# c_dif$allorno = numeric(nrow(c_dif))
# 
# # Get sinlge bar graph
# mean_pv_diff = mean(c_dif$pv_diff)
# ster_pv_diff = st.err(c_dif$pv_diff)
# mean_ts_diff = mean(c_dif$ts_diff)
# ster_ts_diff = st.err(c_dif$ts_diff)
# 
# all_row = c(100, 1,1, mean_pv_diff, mean_ts_diff, mean_pv_diff -ster_pv_diff, mean_pv_diff + ster_pv_diff,
#             mean_ts_diff -ster_ts_diff, mean_ts_diff + ster_ts_diff,1)
# 
# c_dif <- rbind(c_dif, all_row) 
...
# aov_agg2_tr = aggregate(data= prb_dat,gut_norm ~Subj_id + Env + probe_trialno, mean )
# # pv_prb_agg.mdl =
# gut_prb_agg2.aov <- with(data=aov_agg2_tr,  aov(gut_norm~ Env+probe_trialno+ Error(Subj_id)))
# 
...
ggplot(data= data) + geom_point(aes(x=pk_vel, y= force_rate_max, color=factor(Env)))+facet
...
# Next trial for grip realease time
