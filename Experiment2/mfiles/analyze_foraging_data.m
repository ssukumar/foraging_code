...
if nargin<1 
    pilot = 0;
    filename_low = 'foraging_rewd_new_278_low';
    filename_high = 'foraging_rewd_new_278_high';
...
%     clear total_score;
    ...
    params_env = {'ntrials'};
    scalar_metrics = {'alpha_vals','harvest_duration', 'travel_duration',...
        ...
        'avg_tbh','norm_pv', 'norm_td', 'norm_hd','mid_15_trial',...
    ...
    for tr = 1:num_trials
        ...
        if ~isempty(find(tr==ntrials, 1))
            ...
            % We will also flag the trials that fall in the mid 15 minutes in
            % each environment
            if tr == 1
                time_ms_begin = time_ms(1);
            end
            
            % works only for time limited environments of 20 minutes
            if (time_ms(1) - time_ms_begin)/1000 >150 &&...
                    (time_ms(1) - time_ms_begin)/1000 < 1050
                data_subj{fn}.mid_15_trial(tr)=1;
            else 
                data_subj{fn}.mid_15_trial(tr)=0;
            end
...
%                 % determining what happens on those discarded trials
    ...
    [ratios, fig_ix] = diagnostics_ratio_patches(alpha_arr, fn, fig_ix);
...
time_first_harvest = time_ms(find(score(harvest_ok_ix:end)>0,1,'first')+ ...
    harvest_ok_ix-1);
data_subj{fn}.time_last_berry(tr) = time_ms(find(diff(score(harvest_ok_ix:end))...
    > 0,1,'last')+harvest_ok_ix);


% plot(time_ms,score);
% 
% hold on 
% plot(time_first_harvest, score(find(score(harvest_ok_ix:end)>0,1,'first')+ ...
%     harvest_ok_ix-1), 'b*');
% hold on
% plot(data_subj{fn}.time_last_berry(tr), score(find(diff(score(harvest_ok_ix:end))...
%     > 0,1,'last')+harvest_ok_ix), 'g*');
% 
% drawnow
% keyboard
...
%     end
    ...
    if isempty(grip_pk_ix) || isempty(grip_begin_ix) 
        keyboard;
    end
...
if tr >1 && ~isnan(data_subj{fn}.time_last_berry(tr-1))
    data_subj{fn}.time_bw_patch_harvests(tr) = time_first_harvest -...
        data_subj{fn}.time_last_berry(tr-1);
end

data_subj{fn}.harvest_duration(tr) = time_ms(harvest_no_ix) - time_ms(first_har_ok_ix);
% data_subj{fn}.giving_up_time
...
figure(fig_ix)
plot_raster_berries(data_subj,clrs);
hold on
...
%     
%     if isempty(find(vel_profile_rep(1:pk_vel_ix_rep) < 0 ,1,'last'))
%         onset_ix_rep = 
...
data_subj{fn}.max_distance(tr) = max(x_profile_rep(onset_ix_rep:offset_ix_rep)) -...
    min(x_profile_rep(onset_ix_rep:offset_ix_rep));
...
% hold on
% plot(onset_ix_rep, data_subj{fn}.x_profile_rep{tr}(onset_ix_rep),'r^');
% hold on
% plot(offset_ix_rep ,data_subj{fn}.x_profile_rep{tr}(offset_ix_rep),'g^');
% hold off
% drawnow
% keyboard
...
function [ratios, fig_ix] = diagnostics_ratio_patches(alpha_arr, fn, fig_ix)
% Arguments: alphas from the data 
% Check the environemnt and see if the ratios are what you put in 
...
fprintf('\nRatio of patches seen in %s effort env\n', env); 
ratios = zeros(size(alphas));
for a = alphas 
   ratios(alphas==a) = round(sum(alpha_arr==a),2);
end


ratios = ratios./length(alpha_arr)
% keyboard

% determine hwo patches change decision making and vigor by subject

trials_low = find(alpha_arr==alphas(end));
trials_probe = find(alpha_arr==alphas(end-1));
trials_high = find(alpha_arr==alphas(1));

pk_vel_low_mean = nanmean(data_subj{fn}.pk_vel(trials_low));
pk_vel_probe_mean = nanmean(data_subj{fn}.pk_vel(trials_probe));
pk_vel_high_mean = nanmean(data_subj{fn}.pk_vel(trials_high));

clrs = {'b','r'};

figure(fig_ix);
subplot(121);
means = [pk_vel_low_mean,pk_vel_probe_mean,pk_vel_high_mean];
plot( means , [clrs{fn},'*-'], 'Linewidth', 3);
hold on; 
pk_vel_low_sd = nanstd(data_subj{fn}.pk_vel(trials_low));
pk_vel_probe_sd = nanstd(data_subj{fn}.pk_vel(trials_probe));
pk_vel_high_sd = nanstd(data_subj{fn}.pk_vel(trials_high));

errbars = [pk_vel_low_sd, pk_vel_probe_sd, pk_vel_high_sd];

errorbar(means, errbars,[clrs{fn},'*']);

xlabel('Patch type');
ylabel('Vigor toward patch');

hold on;

subplot(122);
harvest_duration_low_mean = nanmean(data_subj{fn}.harvest_duration(trials_low));
harvest_duration_probe_mean = nanmean(data_subj{fn}.harvest_duration(trials_probe));
harvest_duration_high_mean = nanmean(data_subj{fn}.harvest_duration(trials_high));

clrs = {'b','r'};

figure(fig_ix);
means = [harvest_duration_low_mean,harvest_duration_probe_mean,harvest_duration_high_mean];
plot( means , [clrs{fn},'*-'], 'Linewidth', 3);
hold on; 
harvest_duration_low_sd = nanstd(data_subj{fn}.harvest_duration(trials_low));
harvest_duration_probe_sd = nanstd(data_subj{fn}.harvest_duration(trials_probe));
harvest_duration_high_sd = nanstd(data_subj{fn}.harvest_duration(trials_high));

errbars = [harvest_duration_low_sd, harvest_duration_probe_sd, harvest_duration_high_sd];

errorbar(means, errbars,[clrs{fn},'*']);

xlabel('Patch type');
ylabel('Harvest Duration in patch');
